...
# =============================================================================
#  Auto-loaded from .zshrc: $ZSH_CUSTOM=$DOTFILES/config/zsh (themes/plugins/helpers)
#
#  Usage:
...
wiz-k8s-debug() {
  ...
  local pod_full
  pod_full="$(kubectl get pods -n "$ns" --no-headers 2>/dev/null | awk '{print $1}' | grep -F "$pod" | head -1)"
  if [[ -z "$pod_full" ]] && kubectl get pod "$pod" -n "$ns" &>/dev/null; then
    pod_full="$pod"
  fi
  if [[ -z "$pod_full" ]]; then
    echo "  ⚠ No pod matched that filter (partial or exact name). Try a different substring."
    return 1
  fi
  if [[ "$pod_full" != "$pod" ]]; then
    echo "  → Using resolved pod name: $pod_full"
  fi
  pod="$pod_full"
