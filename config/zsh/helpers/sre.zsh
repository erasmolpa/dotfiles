...
# =============================================================================
#  Auto-loaded from .zshrc: source $DOTFILES/config/zsh/helpers/*.zsh
...
help-all() {
  ...
  case "$filter" in
    all)
      echo "\nHelpers (sre.zsh):"
      grep -E '^([a-zA-Z0-9_-]+)\(\) ' "$DOTFILES/config/zsh/helpers/sre.zsh" | awk -F'(' '{print "  - "$1}'
      echo "\nWizards (wizards.zsh):"
      grep -E '^wiz-[a-zA-Z0-9_-]+\(\) ' "$DOTFILES/config/zsh/wizards/wizards.zsh" | awk -F'(' '{print "  - "$1}'
    ...
    scaffolding)
      ...
      grep -E 'tf-new-|gen-|wiz-tf-|wiz-k8s-' "$DOTFILES/config/zsh/helpers/sre.zsh" "$DOTFILES/config/zsh/wizards/wizards.zsh" | awk -F'(' '{print "  - "$1}' | sort | uniq
      ;;
    terraform)
      echo "\nTerraform helpers/wizards:"
      grep -Ei 'tf-|terraform' "$DOTFILES/config/zsh/helpers/sre.zsh" "$DOTFILES/config/zsh/wizards/wizards.zsh" | awk -F'(' '{print "  - "$1}' | sort | uniq
      ;;
    aws)
      echo "\nAWS helpers/wizards:"
      grep -Ei 'aws-' "$DOTFILES/config/zsh/helpers/sre.zsh" "$DOTFILES/config/zsh/wizards/wizards.zsh" | awk -F'(' '{print "  - "$1}' | sort | uniq
      ;;
    prometheus|prom)
      echo "\nPrometheus helpers/wizards:"
      grep -Ei 'prom|pql' "$DOTFILES/config/zsh/helpers/sre.zsh" "$DOTFILES/config/zsh/wizards/wizards.zsh" | awk -F'(' '{print "  - "$1}' | sort | uniq
      ;;
    loki)
      echo "\nLoki helpers/wizards:"
      grep -Ei 'loki' "$DOTFILES/config/zsh/helpers/sre.zsh" "$DOTFILES/config/zsh/wizards/wizards.zsh" | awk -F'(' '{print "  - "$1}' | sort | uniq
      ;;
    k8s|kubernetes)
      echo "\nKubernetes helpers/wizards:"
      grep -Ei 'k8s|kube|klog|kdn|kexec|kpf|krun|kctx|kns|kpod|kall|ksecret|krollout|krestart|kscale|kyaml|kjson' "$DOTFILES/config/zsh/helpers/sre.zsh" "$DOTFILES/config/zsh/wizards/wizards.zsh" | awk -F'(' '{print "  - "$1}' | sort | uniq
      ;;
    docker)
      echo "\nDocker helpers/wizards:"
      grep -Ei 'docker' "$DOTFILES/config/zsh/helpers/sre.zsh" "$DOTFILES/config/zsh/wizards/wizards.zsh" | awk -F'(' '{print "  - "$1}' | sort | uniq
      ;;
    *)
      echo "\nHelpers/wizards matching '$filter':"
      grep -Ei "$filter" "$DOTFILES/config/zsh/helpers/sre.zsh" "$DOTFILES/config/zsh/wizards/wizards.zsh" | awk -F'(' '{print "  - "$1}' | sort | uniq
      ;;
  esac
}
